    ...
    <v-container fluid>
      <v-row class="flex-wrap-reverse">
        ...
        <v-col xs="12" md="6" order="2">
          <p id="solution">
            <!-- $\def\e{\textrm e}$  -->
            Separando as variáveis, $ \color{green}{ (3+2y) \, dy = (2-\exp(x))
            \, dx},$ e integrando a ambos lados, obtemos que as soluções
            $y=f(x)$ da equação diferencial moram nas curvas de nível da função
            $$ \color{green}{V(x,y) = y^2+3y-2x + \exp(x)}.$$ (ver figura).
            ...
            fórmula quadrática, chegando-se a
          </p>
        </v-col>
      </v-row>
      ...
      <v-row>
        <p>
          $$\color{green}{ y = \frac{-3\pm\sqrt{9-4\Delta}}{2}
          =\frac{-3\pm\sqrt{(9 -4C)+8x -4\e^x}}{2}}\,\textrm{(eq1)} $$ Vemos que
          a curva de nível $V(x,y)=C$ esconde duas funções $y=f(x).$ Para
      ...
      <v-row class="flex-wrap-reverse">
        <v-col xs="12" md="6" order="4">
          <p id="solution">
            ...
            somente se $\color{green}{x= \ln(2)\approx 0.69.}$
            <br />
      ...
      <v-row wrap>
        ...
        <v-col xs="12" md="6">
          <p id="solution">
            ...
            (eq1) que $y(x)$ estará definida para valores de $x$ tais que
            $\color{green}{ (13 + 8x)-4\exp(x) \ge 0. }$ Na figura podemos notar
            que aproximadamente no intervalo $\color{green}{[-1.5, 1.97]}$ a
            função $y=13+8x$ está acima de $y=4\exp(x)$ e portanto $y(x)$ está
            bem definida nesse intervalo.
            <br />
